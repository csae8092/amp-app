function leafletDatatable(table, panesShow, panesHide) {
  ...
  // leaflet map:
  var latStart = document.body
    .querySelectorAll(".map-coordinates")[0]
    .getAttribute("lat");
  var longStart = document.body
    .querySelectorAll(".map-coordinates")[0]
    .getAttribute("long");
  ...
  var objects = new L.GeoJSON.AJAX([`geo/${table}.geojson`], {
    onEachFeature: popUp,
  });
  ...
  function getCoordinates() {
    document.body.querySelectorAll(".map-coordinates").forEach(function (node) {
      ...
      if (node.getAttribute("data-country")) {
        var country = node.getAttribute("data-country");
      } else {
        var country = "";
      }
      var place = `${label}<br/>${subtitle}<br/>${country}<br/><a href="${id}.html">Read more</a>`;
  ...
  function popUp(f, l) {
    ...
    if (id) {
      out.push(
        `<a href='${id}.html'>${plc}</a>` + ", " + f.properties["country_code"]
      );
    } else {
